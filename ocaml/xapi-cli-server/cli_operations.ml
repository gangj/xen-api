...
let tunnel_create printer rpc session_id params =
  ...
  in
  let cross_server = get_bool_param params ~default:false "cross_server" in
  let tunnel =
    Client.Tunnel.create ~rpc ~session_id ~transport_PIF:pif ~network ~cross_server ~protocol
  in
